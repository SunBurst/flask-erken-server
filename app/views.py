...
@app.route('/api/profile_measurements_by_location_chart/<string:location_id>/<string:parameter_id>/<int:qc_level>/<int:from_timestamp>/<int:to_timestamp>')
def get_profile_measurements_by_location_chart(location_id, parameter_id, qc_level, from_timestamp, to_timestamp):
    query = "SELECT * FROM profile_measurements_by_location_time WHERE location_id=? AND parameter_id=? AND qc_level=? AND month_first_day=? AND timestamp>=? AND timestamp<=? ORDER BY timestamp ASC"
    prepared = cassandra_connection.session.prepare(query)
    ...
    for future in futures:
        ...
        for row in rows:
            ...
            stations[station_id]['data'].append([row.get('timestamp'), row.get('depth'), row.get('value')])
